...
import { getMaterials, getMaterialVariants, createMaterialDelivery } from '@/actions/materials';
...
export default function AddMaterialPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [mats, parts, bldgs, vehs, wbs] = await Promise.all([
          getMaterials(),
          getParties(),
          getBuildings(),
        ...
        setMaterials(mats);
        setSuppliers(parts.filter(p => p.class === 'supplier' || p.class === 'shop'));
        setBuildings(bldgs);
        setVehicles(vehs);
        setWeighbridges(wbs);
      } catch (err) {
  ...
  const weighbridgeOptions = weighbridges.map(w => ({ id: w.id, label: w.name }));
