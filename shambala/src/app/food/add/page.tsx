...
export default function AddFoodPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        setShops(parties.filter(p => p.class === 'shop' || p.class === 'supplier'));
  ...
  const categoryOptions = foodCategories.map(c => ({ id: c.id, label: c.name }));
  const shopOptions = shops.map(s => ({ id: s.id, label: s.name }));
